...
{
    printf("Beginning GobalSFM::construct(...)");
    ...
    {
        //solve pnp
        printf("solve pnp frame: %d", i); // cout << "solve pnp frame " << i << endl;
        Matrix3d R_initial = c_Rotation[i + 1];
        Vector3d P_initial = c_Translation[i + 1];
...
//    options.max_solver_time_in_seconds = 0.3; TODO
    ceres::Solver::Summary summary;
    ceres::Solve(options, &problem, &summary);
    std::cout << summary.BriefReport() << "\n";
    if (summary.termination_type == ceres::CONVERGENCE || summary.final_cost < 3e-03){
        printf("vision only BA converge"); // cout << "vision only BA converge" << endl;
    }
    else
    {
        printf("vision only BA not converge"); // cout << "vision only BA not converge " << endl;
        return false;
    }
    for (int i = 0; i < frame_num; i++)
    {
