...
//    //外参虽然参与优化，外参本身没有进行优化
//    LOGI("77777 :%lf,%lf,%lf", tic.x(), tic.y(), tic.z());
...
void VINS::update_loop_correction() {
    //update loop correct pointcloud,更新云点，为了显示在手机上
...
//        len = sqrt(pow(correct_Ps[i][0], 2)+
//                   pow(correct_Ps[i][1], 2)+
//                   pow(correct_Ps[i][2], 2));
//
//        LOGI("77777 :%d  距离： %lf",i,len);
    }
}
...
//            LOGI("%d,%lf,%lf,%lf",j,un_acc_0[0],un_acc_0[1],un_acc_0[2]);
...
void VINS::processImage(map<int, Vector3d> &image_msg, double header, int buf_num) {
    ...
    LOGI_VINS("vinsmonodebug, beforeinit");
    if (solver_flag == INITIAL) {
        ...
        LOGI_VINS("vinsmonodebug, init1");
        if (frame_count == WINDOW_SIZE) {
            if (track_num < 20) {
                clearState();
                LOGI_VINS("vinsmonodebug, init2");
                return;
            }
            bool result = false;
            if (header - initial_timestamp > 0.3) {
                LOGI_VINS("vinsmonodebug, init3");
                result = solveInitial();
                LOGI_VINS("vinsmonodebug, init4");
                initial_timestamp = header;
            }
            if (result) {
                LOGI_VINS("vinsmonodebug, init5");
                solve_ceres(buf_num);
                LOGI_VINS("vinsmonodebug, init6");
                if (final_cost > 200)  //initialization failed, need reinitialize,重新初始化
                {
                    LOGI_VINS("vinsmonodebug, init7");
                    ...
                    fail_times++;
                    slideWindow();
                    LOGI_VINS("vinsmonodebug, init8");
                } else {
                    LOGI_VINS("vinsmonodebug, init9");
                    ...
                    last_P_old = Ps[0];
                    TE(Initialisierung);
                    LOGI_VINS("vinsmonodebug, init10");
                }
            } else {
                slideWindow();
            }
        ...
        LOGI_VINS("vinsmonodebug, finishInit");
...
void VINS::solve_ceres(int buf_num) {
    ...
    //camera imu的外参也参与优化，外参本身并没有参与优化
    for (int i = 0; i < NUM_OF_CAM; i++) {
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
    ...
    //last_marginalization_parameter_blocks 里保存的就是跟当前窗口有约束的变量
    if (last_marginalization_info != nullptr) {
    ...
    //IMU factor   IMU残差
    //反复使用AddResidualBlock方法
    //将每个imu的残差累计求和构建最小二乘优化式
    for (int i = 0; i < WINDOW_SIZE; i++) {
        int j = i + 1;
    ...
    //for marginalization back   优化后边缘化处理
