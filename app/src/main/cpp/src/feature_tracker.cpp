...
void FeatureTracker::readImage(const cv::Mat &_img, cv::Mat &result, int _frame_cnt,
                               vector<Point2f> &good_pts, vector<double> &track_len, double header,
                               Vector3d &P, Matrix3d &R, bool vins_normal) {
    ...
    //track
    {
        if (cur_pts.size() > 0) {
            ...
            //光流，将上一帧的特征点，映射到下一帧上，保存为forw_pts
            TS(time_track);
            calcOpticalFlowPyrLK(cur_img, forw_img, cur_pts, forw_pts, status, err,
                                 cv::Size(21, 21), 3);
            TE(time_track);
...
//                    cv::line(result, pre_pts[i], forw_pts[i], cvScalar(0), 3, 8, 0);
                    ...
                    double parallax = (cv::norm(parallax_cnt[i].max - parallax_cnt[i].min) < 2.0 ?
                                       0 : cv::norm(parallax_cnt[i].max - parallax_cnt[i].min));
