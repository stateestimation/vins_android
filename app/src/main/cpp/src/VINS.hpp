...
#define LOGI_VINS(...)  __android_log_print(ANDROID_LOG_INFO, "vins", __VA_ARGS__)
...
public:
    ...
    Vector3d Ps[10 * (WINDOW_SIZE + 1)];    //平移
    Vector3d Vs[10 * (WINDOW_SIZE + 1)];    //速度
    Matrix3d Rs[10 * (WINDOW_SIZE + 1)];    //旋转
    Vector3d Bas[10 * (WINDOW_SIZE + 1)];   //加速度偏差
    Vector3d Bgs[10 * (WINDOW_SIZE + 1)];   //陀螺仪偏差

    //ceres参数块
    double para_Pose[WINDOW_SIZE + 1][SIZE_POSE];       //位置
    double para_SpeedBias[WINDOW_SIZE + 1][SIZE_SPEEDBIAS];     //速度
    double para_Feature[NUM_OF_F][SIZE_FEATURE];        //特征点深度
    ...
    Vector3f correct_Ps[WINDOW_SIZE];       // 真实位置
