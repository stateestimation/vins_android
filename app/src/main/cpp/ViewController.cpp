...
std::vector<std::pair<std::vector<ImuConstPtr>, ImgConstPtr>> ViewController::getMeasurements() {
    ...
    while (true) {
        ...
        //首个imu数据  后于 首个图像数据，弹出首个图像数据，保证IMU数据必须在img数据之前
        ...
        if (LOOP_CLOSURE) {
            ...
            if (vins.marginalization_flag == vins.MARGIN_OLD &&
                ...
                if (!first_frame && keyframe_freq % LOOP_FREQ == 0) {
                    ...
                    m_image_buf_loop.lock();
                    //保证image_buf_loop里只有最后两帧图像,index为8和9的两帧图像
...
int ViewController::process_imu_sensor_events(int fd, int events, void *data) {
    ...
    //保存数据
    if (GET_DATA && first) {
        outfile.open("/storage/emulated/0/Android/data/com.example.vins/files/imu0.csv", ios::ate);
        first = false;
        outfile << "timestamp";
        outfile << ",";
        outfile << "omega_x";
...
//            imu_msg->header = (double)cv::getTickCount() / cv::getTickFrequency();
        ...
        if (GET_DATA) {
            ostringstream oss;
            oss << gyroEvent.timestamp << ","
                << gyroEvent.uncalibrated_gyro.x_uncalib << ","
                << gyroEvent.uncalibrated_gyro.y_uncalib << ","
                << gyroEvent.uncalibrated_gyro.z_uncalib << ","
                << acclEvent.acceleration.x << ","
                << acclEvent.acceleration.y << ","
                << acclEvent.acceleration.z << "\n";
